...
int MatrixGesture::begin(void) {
    ...
    for (uint16_t i = 0;
         i < sizeof(initRegisterArray) / sizeof(initRegisterArray[0]); i++) {
...
uint8_t MatrixGesture::readReg(uint8_t reg, void *pBuf, size_t size) {
    ...
    for (uint16_t i = 0; i < size; i++) _pBuf[i] = _pWire->read();
    _pWire->endTransmission();
    return size;
}

const MatrixGesture::sGestureDescription_t
